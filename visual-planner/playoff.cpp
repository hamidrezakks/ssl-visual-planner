                 ...
                 QWidget(parent)
{
    ...
    myPlan = new kkPOPlanSQL("temppo.db");
...
void playoff::setSkill(POffSkills tSkill, int targetAgent, int targetIndex)
{
    if(currentRobot.agent != -1 && currentRobot.index != -1)
    {
        if(robots[currentRobot.agent].count()-1 >= currentRobot.index)
        {
            ...
            //if(tSkill == ShotToGoalSkill || tSkill == ChipToGoalSkill || tSkill == OneTouchSkill)
                draw();
        }
    }
}
        ...
        for(int j = 0; j < robots[i].length(); j++)
        {
            for(int k = 0; k < robots[i].at(j).skillSize; k++)
            {
                if(robots[i].at(j).skill[k] == PassSkill ||
                   ...
                   robots[i].at(j).skill[k] == OneTouchSkill)
                {
                    flag = true;
...
void playoff::apply(int _id)
{
    POInitPos tempInit;
    tempInit = getInitPos();
    qDebug()<<myPlan->addPlan(robots, tempInit, currentPOMode, agentSize, _id);
}
...
{
    myPlan->changeSQLDir(directory);
    return myPlan->loadPlan();
}

void playoff::loadPOPlan(int index)
{
...
void playoff::clean()
{
    POSetSelectedModeLabel(KICKOFF);
    currentPOMode = KICKOFF;
